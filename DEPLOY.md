# ThreatQuest Cyber Hunt India — Setup & Deployment Guide
...
```bash
cd ThreatQuest Cyber Hunt/backend
...
- **Cron** (Linux/Mac): `0 8 * * * cd /path/ThreatQuest Cyber Hunt/backend && python reminders.py`
...
3. When prompted, fill the secret env vars (these are the `sync:false` ones):
   - `APP_URL` = your Render URL (e.g. `https://ThreatQuest Cyber Hunt.onrender.com`)
...
2. Render → **New → Web Service** → connect the repo, root = `ThreatQuest Cyber Hunt/backend`.
...
**Today:** the app uses `ThreatQuest Cyber Hunt.db` (SQLite). This is genuinely fine for thousands of users.
