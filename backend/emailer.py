"""
ThreatQuest Cyber Hunt India — email reminders.
...
SMTP_FROM = os.environ.get("SMTP_FROM", SMTP_USER or "noreply@ThreatQuest Cyber Hunt.local")
...
RESEND_FROM = os.environ.get("RESEND_FROM", "ThreatQuest Cyber Hunt <onboarding@resend.dev>")
...
def send_email(to, subject, html):
    ...
    msg["Subject"] = subject
    msg["From"] = f"ThreatQuest Cyber Hunt India <{SMTP_FROM}>"
    msg["To"] = to
...
def _shell(headline, sub, cards, cta_label):
    return (
        ...
        f'joined ThreatQuest Cyber Hunt. Open <a href="{APP_URL}" style="color:#8a7bff;text-decoration:none">ThreatQuest Cyber Hunt.xyz</a> '
...
def _render(name, events):
    ...
    return _shell(f"Deadlines closing soon, {first}",
                  ...
                  cards, "Open ThreatQuest Cyber Hunt")
...
def _render_digest(name, events):
    ...
    return _shell(f"This week's top hackathons, {first}",
                  ...
                  cards, "See all on ThreatQuest Cyber Hunt")
...
def run_digest(events):
    ...
    for u in users:
        ...
        if "@" not in email:
            continue
        if send_email(email, "🚀 Top hackathons this week on ThreatQuest Cyber Hunt", _render_digest(u.get("name") or "there", top)):
            sent += 1
        time.sleep(0.6)  # stay under Resend's ~2 requests/sec rate limit
...
def _render_announcement(name):
    ...
    return _shell(f"{first}, India's biggest hackathon just dropped \U0001f680",
                  "We've featured it at the top of ThreatQuest Cyber Hunt — a ₹50 Lakh+ prize pool, free to enter, "
                  "and open to everyone. Don't sit this one out:",
                  card, "Open ThreatQuest Cyber Hunt")
...
def run_announcement():
    ...
    for u in users:
        ...
        if "@" not in email:
            continue
        if send_email(email, "🚀 India Runs by RedRob AI — ₹50 Lakh+ prize, now live on ThreatQuest Cyber Hunt",
