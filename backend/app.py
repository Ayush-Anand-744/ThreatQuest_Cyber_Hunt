...
def _maybe_tier_email(code):
    ...
    try:
        ...
        if not amb or "@" not in (amb.get("email") or ""):
            return
        import emailer
        url = "https://ThreatQuest Cyber Hunt.xyz/ambassador.html"
        html = f"""<div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:auto">
          ...
          <p style="color:#444">You've brought <b>{count} sign-ups</b> to ThreatQuest Cyber Hunt — amazing work!
          ...
          ambassador dashboard. Keep going for the next tier! — Team ThreatQuest Cyber Hunt</p></div>"""
        emailer.send_email(amb["email"], f"🎉 You unlocked {tier} on ThreatQuest Cyber Hunt!", html)
    except Exception as e:
...
OWNER_PASS = os.environ.get("OWNER_PASS", "ThreatQuest Cyber Hunt-secret-2026")
...
def owner_2fa_setup():
    ...
    owner can add ThreatQuest Cyber Hunt to Google Authenticator. Set the secret as
    ...
    label = "ThreatQuest Cyber Hunt%20Owner"
    otpauth = ("otpauth://totp/%s?secret=%s&issuer=ThreatQuest Cyber Hunt&algorithm=SHA1&digits=6&period=30"
               % (label, secret))
...
def security_status():
    ...
    return jsonify({
        "protected": True,
        "engine": "ThreatQuest Cyber Hunt Shield",
...
def _gate():
    ...
    #    host) to https://ThreatQuest Cyber Hunt.xyz. Skip health check so Railway monitoring
    ...
    if p.startswith("/api/") and db.get_setting("maintenance", "0") == "1":
        ...
        if not any(p.startswith(a) for a in allow):
            return jsonify({"maintenance": True,
                            "message": "ThreatQuest Cyber Hunt is temporarily down for maintenance."}), 503
...
def email_test():
    ...
    if fmt == "digest":
        ...
        subject = "🚀 Top hackathons this week on ThreatQuest Cyber Hunt"
        ...
        subject = "🚀 India Runs by RedRob AI — ₹50 Lakh+ prize, now live on ThreatQuest Cyber Hunt"
        html = emailer._render_announcement(name)
    else:
        subject = "ThreatQuest Cyber Hunt test email"
        html = ("<div style='font-family:Inter,Arial,sans-serif'>"
                "<h2 style='color:#7c5cff'>It works! 🎉</h2><p>Your ThreatQuest Cyber Hunt email "
    ...
    req = urllib.request.Request("https://api.resend.com/emails", data=payload, method="POST",
                                 headers={"Authorization": "Bearer " + emailer.RESEND_API_KEY,
                                          ...
                                          "User-Agent": "ThreatQuest Cyber Hunt/1.0 (+https://ThreatQuest Cyber Hunt.xyz)",
...
def _seo_block():
    ...
    for i, e in enumerate(evs):
        ...
        url = e.get("url") or "https://ThreatQuest Cyber Hunt.xyz/"
    ...
    crawl = ('<section aria-hidden="false" style="position:absolute;width:1px;height:1px;'
             ...
             'across India with deadlines and direct apply links on ThreatQuest Cyber Hunt.</p>%s<ul>%s</ul></section>'
...
def robots():
    body = ("User-agent: *\nAllow: /\n"
            "Disallow: /owner.html\nDisallow: /api/owner\n"
            "Sitemap: https://ThreatQuest Cyber Hunt.xyz/sitemap.xml\n")
...
def sitemap():
    ...
    def u(p, prio, freq):
        return ('<url><loc>https://ThreatQuest Cyber Hunt.xyz%s</loc><lastmod>%s</lastmod>'
...
<meta property="og:image" content="https://ThreatQuest Cyber Hunt.xyz/icon-512.png">
...
def _list_page(h1, intro, evs):
    ...
    return ("<a class='back' href='/'>&larr; ThreatQuest Cyber Hunt</a>"
            "<div class='brand'>Hack<span class='grad'>Hunt</span></div>"
            "<h1>%s</h1><p class='meta'>%s</p><div class='list'>%s</div>"
            "<div class='cta'><a class='btn primary' href='/'>Open the live ThreatQuest Cyber Hunt feed &rarr;</a></div>"
...
def seo_event(eid):
    try:
        ...
        if not e:
            return redirect("/")
        canon = "https://ThreatQuest Cyber Hunt.xyz/hackathon/" + _html.escape(key)
        ...
        apply_url = _html.escape(str(e.get("url") or e.get("ticket_url") or "https://ThreatQuest Cyber Hunt.xyz/"))
        body = ("<a class='back' href='/'>&larr; ThreatQuest Cyber Hunt</a>"
                ...
                "<p class='note'>Listed on ThreatQuest Cyber Hunt &mdash; every hackathon, hiring challenge and tech "
        ...
        desc = (str(e.get("organizer") or "") + " " + str(e.get("category") or "Hackathon")
                + " in India. Dates, deadline & direct apply link on ThreatQuest Cyber Hunt.")
        return _render_seo(t + " — Apply | ThreatQuest Cyber Hunt", desc, canon, body,
...
def seo_city(city):
    try:
        ...
        canon = "https://ThreatQuest Cyber Hunt.xyz/in/" + _slug(cs)
        disp = cs.title()
...
if __name__ == "__main__":
    print("ThreatQuest Cyber Hunt India running at http://localhost:5050")
