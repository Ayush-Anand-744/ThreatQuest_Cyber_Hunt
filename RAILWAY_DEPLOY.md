# Deploy ThreatQuest Cyber Hunt India to Railway

## 1. Push to GitHub
```bash
cd ~/Desktop/ThreatQuest Cyber Hunt
git config --global user.name "Pranay"          # one-time
git config --global user.email "sonugoenka40@gmail.com"
git init
git add .
git commit -m "ThreatQuest Cyber Hunt India — initial deploy"
git branch -M main

# with GitHub CLI:
gh repo create ThreatQuest Cyber Hunt --public --source=. --push
# OR manually (after creating an empty repo at github.com/new named "ThreatQuest Cyber Hunt"):
git remote add origin https://github.com/YOUR_USERNAME/ThreatQuest Cyber Hunt.git
git push -u origin main
```
`.env`, `ThreatQuest Cyber Hunt.db`, `cache.json` are gitignored — no secrets are pushed.

## 2. Railway
1. railway.app → **New Project → Deploy from GitHub repo** → choose `ThreatQuest Cyber Hunt`.
2. Service → **Settings**:
